...
export const wrappedCurrencyAmount = (currencyAmount: CurrencyAmount | undefined, chainId: ChainId | undefined): TokenAmount | undefined => {
  const token = currencyAmount && chainId ? wrappedCurrency(currencyAmount.currency, chainId) : undefined;
  return token && currencyAmount ? new TokenAmount(token, currencyAmount.raw) : undefined;
};

export const unwrappedToken = (token: Token): Currency => (token.equals(WETH[token.chainId]) ? ETHER : token);
